...
const Meeting = () => {
  ...
  useEffect(() => {
    const fetchMeetings = async () => {
      try {
        const response = await fetch('http://localhost:4518/gknbvg/SkillPort-recruiter/ertqyuiok/all-meetings');
        const data = await response.json();
        if (data && Array.isArray(data.data)) {
          const formattedMeetings = data.data.map((meeting) => ({
            id: meeting._id,
            title: meeting.interviewTitle,
            company: meeting.jobId?.jobtitle || 'N/A',
            candidate: meeting.candidateId?.Fullname || 'N/A',
            recruiter: meeting.recruiterId?.Fullname || 'N/A',
            scheduledAt: meeting.interviewDate,
            durationMinutes: meeting.duration,
            meetingType: meeting.interviewType,
